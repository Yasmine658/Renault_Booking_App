...
const handleLogin = async (e: React.FormEvent) => {
  ...
  try {
    ...
    if (err.response?.status === 400) {
      toast.error("Compte non trouvé. Vous allez être redirigé vers la page d'inscription.");
      setTimeout(() => {
        navigate("/signup");
      }, 2000); 
    } else {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <div className="w-2/3 bg-gray-100 p-8 relative">
        ...
        <h1 className="text-3xl font-bold mt-30 text-gray-800 mb-20 flex justify-center">Se connecter</h1>
        <form onSubmit={handleLogin} className="mx-80 mt-30 space-y-4">
          ...
          <Button className="w-full mt-30">Connexion</Button>
        </form>
      </div>
    </div>
  );
